...
const planName = (plan) => plan.displayName || plan.id;
function membership(m) {
  return (
    "<h3>" +
    esc(
      planName(
        [catalog.freePlan, ...catalog.plans].find((p) => p.id === m.planId) || {
          id: m.planId,
        },
      ),
    ) +
    "</h3><p>Valid until: " +
    esc(m.grantEndsAt || "No fixed expiry") +
    '</p><div class="table"><table><thead><tr><th>Capability</th><th>Used</th><th>Limit</th><th>Period</th></tr></thead><tbody>' +
    Object.entries(m.capabilities)
      .map(
        ([key, v]) =>
          ...
          esc(v.used) +
          "</td><td>" +
          esc(v.limit === null ? "Unlimited" : v.limit) +
          "</td><td>" +
          esc(v.period) +
          "</td></tr>",
      )
...
function home() {
  content.innerHTML =
    '<h2>Find a user</h2><p>Look up an exact login subject or payment identity.</p><form id="search"><label for="query">Subject or payment identity</label><input id="query" name="query" required maxlength="200" autocomplete="off"><div class="actions"><button type="submit">Find user</button>' +
    (context.canConfigure
      ? '<button type="button" class="secondary" id="plans">Manage plans and quotas</button>'
      : "") +
    "</div></form>";
  bindForm("search", async (f) => {
    await showUser(f.get("query"));
  });
...
async function showUser(query) {
  ...
  content.innerHTML =
    ...
    "</code></p>" +
    (user.appUserId
      ? "<p>Payment identity: <code>" + esc(user.appUserId) + "</code></p>"
      : "") +
    "<p>Effective source: " +
    esc(
      {
...
async function configure() {
  try {
    ...
    content.innerHTML =
      '<button class="secondary" id="cancel">Back to search</button><h2>Plans and quotas</h2><p>Names and limits apply on the next membership read. Leave a limit blank for unlimited. Prices remain store-owned.</p><form id="catalog-form">' +
      plans
        .map(
          (p, i) =>
...
async function start() {
  try {
    context = (await api("/context")).data;
    document.getElementById("identity").innerHTML =
      '<span class="badge">' +
      esc(context.environment) +
      "</span><p>Signed in as " +
      esc(context.operator) +
      "</p>";
    document.getElementById("environments").innerHTML =
      context.environments
        .map((e) => '<a href="' + esc(e.url) + '">' + esc(e.name) + "</a>")
        .join("") +
      '<a href="' +
      esc(base) +
      '/session/logout">Sign out</a>';
    home();
    message("Select a user to inspect membership.");
  } catch (e) {
    message(e.message, true);
    content.innerHTML =
      '<h2>Administration access required</h2><p>Use your authorized operator account.</p><a href="' +
      esc(base) +
      '/session/login">Sign in with OIDC</a>';
  }
}
start();
`;
